...
def quantize(y, bits, modem_num):  # uniform quantization
    maxval = torch.max(torch.abs(y))
    k = 2 ** bits
    r = y.clone()
    interval = 2*maxval/k
    for i in range(k):
    ...
    rr = r.clone()
    K = modem_num ** 0.5
    before_val = torch.arange(1-K, K, 2).to(device)
    after_val = torch.mean(before_val.reshape(k, -1), dim=1)
    for i in range(k):
        rr[r == i] = after_val[i]
    return rr
...
class Quantization(nn.Module):
    def __init__(self, bits, modem_num):
    ...
    def forward(self, input, quanmode, T=1):
        ...
        if quanmode == 'train':
            ...
            return quantize(input, self.bits, self.modem_num)
